...
static void print_config_tree(confdb_handle_t handle, hdb_handle_t parent_object_handle, const char *dn, char *fulldn)
{
	...
	if (res != CS_OK) {
		printf( "error resetting key iterator for object "HDB_D_FORMAT": %d\n", parent_object_handle, res);
		return;
	}
	...
	if (res != CS_OK) {
		printf( "error resetting object iterator for object "HDB_D_FORMAT": %d\n", parent_object_handle, res);
		return;
	}
	...
	while ( (res = confdb_object_iter(handle, parent_object_handle, &object_handle, object_name, &object_name_len)) == CS_OK)	{
		...
		if (res != CS_OK) {
			printf( "error getting parent for object "HDB_D_FORMAT": %d\n", object_handle, res);
			return;
		}
